...
public sealed class MainWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        using var tabs = ImRaii.TabBar("##wyu2-tabs");
        if (!tabs.Success)
            return;
    ...
    {
        using var popup = ImRaii.ContextPopupItem("wyu2-friend-menu");
        if (!popup.Success)
            return;
    ...
    private void DrawContacts()
    {
        ...
        ImGui.InputTextWithHint("##sharecode", "WY-XXXX-XXXX-XXXX-XXXX", ref shareCodeInput, 40);
        ImGui.SameLine();
    ...
    private void DrawRelay()
    {
        ImGui.Spacing();
        ImGui.TextWrapped(
            "Wyu2 needs a relay to pass updates between clients. It only ever sees encrypted blobs; " +
    ...
    private void RunAsync(Func<Task> action)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                Service.Log.Error(ex, "Wyu2 UI action failed");
