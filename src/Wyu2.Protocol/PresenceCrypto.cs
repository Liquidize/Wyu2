...
/// <summary>
/// Sealed box for presence payloads: ECDH P-256 to agree a per-pair secret, HKDF-SHA256 to derive a
/// per-direction key, AES-256-GCM to encrypt. The relay only ever sees nonce plus ciphertext, so it
/// cannot read where anybody is.
/// </summary>
public static class PresenceCrypto
{
        ...
        string recipientAccountId)
    {
        ...
        var info = Encoding.UTF8.GetBytes(
            $"{ProtocolConstants.KeyDerivationInfo}|{senderAccountId}|{recipientAccountId}");
    ...
    public static PresenceEnvelope Seal(
        byte[] key,
    ...
    /// </summary>
    public static PresencePayload? Open(
        byte[] key,
        string nonceBase64,
        ...
        string recipientAccountId)
    {
        try
        {
            ...
            return JsonSerializer.Deserialize<PresencePayload>(plaintext, PayloadJson);
        }
