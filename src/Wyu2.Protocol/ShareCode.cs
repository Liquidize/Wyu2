...
public static class ShareCode
{
    ...
    private const string Prefix = "WY";
    private const int SecretBytes = 10;   // 80 bits -> 16 base32 characters
    private const int GroupSize = 4;

    /// <summary>Length of the secret once encoded, without the prefix or the dashes.</summary>
    private const int RawLength = SecretBytes * 8 / 5;

    /// <summary>Mints a fresh random code, e.g. <c>WY-9GQ4-K72M-XPZ1-D8V0</c>.</summary>
    ...
    public static string? Normalize(string? input)
    {
        ...
        if (text.Length == Prefix.Length + RawLength && text.StartsWith(Prefix, StringComparison.Ordinal))
            text = text[Prefix.Length..];

        return text.Length == RawLength ? text : null;
    }
